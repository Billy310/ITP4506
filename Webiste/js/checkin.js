function clickviewboardingrecord(id){
    ...
    // localStorage.setItem("bview",);
    console.log(id);
    // $(location).prop('href', 'downloadboardingpass.html')
}
...
//     var checkbox = `<label class="checkbox">
// <input type="checkbox" class="cb" id="a777" />
...
var checkboxafter=`"onclick="clickviewboardingrecord(this.id)">Download<span><span></span></span></div>`
    ...
    $.getJSON("json/ticket_ver1.json", function (jd) {
        ...
        for (var i = 0; i < jd.ticket.length; i++) {
            if (username == jd.ticket[i].owner) {
                $("#samerecord").append(
                    `<tr id=xx` + hasrecord + `><td>` + checkbox+jd.ticket[i].id+checkboxafter + `</td><td>` +
                    jd.ticket[i].Type +
                    "</td><td>" +
                    jd.ticket[i].seat +
                    "</td><td>"
                    + jd.ticket[i].fullname +
                    "</td><td>"
                    + jd.ticket[i].ppnum + `</td></tr>`);
